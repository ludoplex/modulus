...
class Graph:
    ...
    def __init__(
        ...
        self.max_order = len([key for key in self.icospheres if "faces" in key]) - 2
    ...
    def create_mesh_graph(self, verbose: bool = True) -> Tensor:
        ...
        for i in range(1, self.max_order + 1):
            multimesh_faces = np.concatenate(
                (multimesh_faces, self.icospheres[f"order_{str(i)}_faces"])
            )
        ...
        mesh_pos = torch.tensor(
            self.icospheres[f"order_{str(self.max_order)}_vertices"],
            dtype=torch.float32,
        )
    ...
    def create_g2m_graph(self, verbose: bool = True) -> Tensor:
        ...
        edge_src = self.icospheres[f"order_{str(self.max_order)}_vertices"][
            self.icospheres[f"order_{str(self.max_order)}_faces"][:, 0]
        ]
        edge_dst = self.icospheres[f"order_{str(self.max_order)}_vertices"][
            self.icospheres[f"order_{str(self.max_order)}_faces"][:, 1]
        ]
        ...
        edge_dst = self.icospheres[f"order_{str(self.max_order)}_vertices"][
            self.icospheres[f"order_{str(self.max_order)}_faces"][:, 2]
        ]
    ...
    def create_m2g_graph(self, verbose: bool = True) -> Tensor:
        ...
        n_nbrs = 1
        neighbors = NearestNeighbors(n_neighbors=n_nbrs).fit(
            self.icospheres[f"order_{str(self.max_order)}_face_centroid"]
        )
        ...
        src = [
            p
            for i in indices
            for p in self.icospheres[f"order_{str(self.max_order)}_faces"][i]
        ]
